...
function AdminUsersPage() {
  ...
  // Use _id or id for tenant_id
  const getTenantId = (t: any) => t._id || t.id;
            ...
            {users.length ? (
              users.map((u: any) => (
                <TableRow key={u._id}>
                  ...
                  <TableCell>{tenants.find(t => getTenantId(t) === u.tenant_id)?.name || u.tenant_id}</TableCell>
                </TableRow>
              ))
            ) : (
              <TableRow>
        ...
        <DialogContent>
          ...
          <form className='flex flex-col gap-4 mt-2' onSubmit={e => { e.preventDefault(); handleInvite(); }}>
            ...
            >
              ...
              <SelectContent>
                {tenants.map((t: any) => (
                  <SelectItem key={getTenantId(t)} value={getTenantId(t)}>{t.name}</SelectItem>
                ))}
              </SelectContent>
            </Select>
